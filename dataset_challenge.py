...
class DogsDataset(Dataset):
    ...
    def __init__(self, partition, task="target", augment=False):
        ...
        # Define transformation pipeline with augmentation if needed
        base_transform = [
            transforms.Resize((64, 64)),  # Resize images to match model input dimensions
            transforms.ToTensor(),
            transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))  # Normalize to match training data distribution
        ]
        ...
        if self.augment and self.partition == "train":
            ...
            self.transform = transforms.Compose(augmentation_transforms + base_transform)
        else:
