...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        norm_x = self.norm1(x)
        attention_output = self.multi_head_attention(norm_x)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for sequence in x:
            ...
            for head in range(self.num_heads):
                ...
                # step 3: Apply the softmax function to the scaled scores
                attention_weights = F.softmax(scores, dim=-1)
                
                # step 4: Apply the attention weights to the value matrix
...
class ViT(nn.Module):
    ...
    def forward(self, X: torch.Tensor) -> torch.Tensor:
        ...
        for transformer in self.transformer_blocks:
            all_tokens = transformer(all_tokens)
        # TODO: END
